...
export function NavBar () {
    return(
        <motion.div initial={{ opacity: 0.1 }} animate={{ opacity: 1 }} transition={{ duration: 2 }} className="navBar">
            <motion.div initial={{ scale: 5, x: "43vw", y: "50vh" }} animate={{ scale: 1 , x: 0, y: 0 }}  transition={{ duration: 1, ease: "easeIn", delay: 2}} id="icon">
                <motion.img src={navLogo} initial={{ rotate: 0 }} animate={{ rotate: 360 }} transition={{duration: 2, delay:0.5}} whileHover={{ borderRadius: "20%" }} />
                <motion.h3 initial={{ scale: 2 }} animate={{ scale: 1 }} transition={{ type: "spring" }}>Sung</motion.h3>
            </motion.div>
            <div id="navBar-link">
